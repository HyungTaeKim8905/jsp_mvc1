...
<%
	...
	try {
		...
		String query = "insert into qnaboard(qnatitle, qnacontent, qnawdate, id, QNAwriter, filename, filepath, filerealname) values(?, ?, now(), ?, ?, ?, ?, ?)";
		...
		query = "update qnaboard set pano = QNAno where pano = 0 order by QNAno desc limit 5";
		pstmt = conn.prepareStatement(query);
		pstmt.executeUpdate();
